...
# 제목
st.title("🤖 유니버설로봇 Realtime Data 분석 AI Agent")
st.markdown("---")
...
if st.session_state.analyzed and st.session_state.results is not None:
    ...
    with col1:
        st.metric(label="전체 상태", value=summary['overall_status'].split()[0])
    
    with col2:
        st.metric(label="속도 사용률", value=f"{summary['speed_utilization']}%")
    
    with col3:
        st.metric(label="평균 부하율", value=f"{summary['load_ratio']}%")
    
    with col4:
        st.metric(label="모션 효율", value=f"{summary['motion_efficiency']}%")
    
    with col5:
        st.metric(label="발견된 이슈", value=f"{summary['issues_count']}건")
    ...
    if uploaded_file is None:
        ...
        with st.expander("📖 사용 가이드", expanded=True):
            ...
            - `actual_qd_0` ~ `actual_qd_5`: 관절 속도 (rad/s)
